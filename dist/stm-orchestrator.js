...
class STMOrchestrator {
    ...
    orchestrateActuation(layer2Data, emergencyToken, historicalPlans) {
        ...
        if (emergencyToken) {
            ...
            reasonChain.push(`Using EMERGENCY_MODE with phase ${emergencyToken.targetPhaseId}`);
        }
        else {
            // TODO: Member 1 will implement this - for now, use fallback
            reasonChain.push(`Using NORMAL_MODE - would call Member 1 for optimization`);
        ...
        const proposedState = {
            phaseId: "targetPhaseId" in selectedProposal
    ...
    produceFallbackCommand(commandId, junctionId, historicalPlans, mode, reasonChain, confidenceScore) {
        ...
        const fallbackPhase = historicalPlans[0] || { phaseId: "NORTH", recommendedGreenTime: 45, historicalDemand: 60 };
        const command = {
            junctionId,
            commandId,
     ...
     */
    generateNormalModeProposal(layer2Data) {
        ...
        const maxApproach = approaches.reduce((prev, current) => current.spatialOccupancyPct > prev.spatialOccupancyPct
            ? current
            : prev);
        return {
    ...
    generateEmergencyResponse(emergency) {
        ...
        const priorityMultiplier = emergency.priorityClass === "CRITICAL"
            ? 3
